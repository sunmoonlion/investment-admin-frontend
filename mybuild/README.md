# investment-admin-frontend 镜像构建
...
```bash
cd /home/zymun/master/tpl-app/investment-admin-frontend
docker build \
  --progress=plain \
...
```bash
cd /home/zymun/master/tpl-app/investment-admin-frontend/mybuild
./build-image.sh --tag 1.0.0
```

`build.conf` 的主要配置：

- `TPL_SSR_IMAGE`：默认 `investment-admin-frontend`
- `TPL_SSR_TAG`：镜像标签
